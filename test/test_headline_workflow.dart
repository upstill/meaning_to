...
void main() {
  group('Headline Auto-Population Workflow', () {
    ...
    test('YouTube link with generic "Watch" title should fetch from webpage',
        () async {
      // This simulates what happens when LinkEditScreen returns a YouTube link with generic title
      const htmlLink =
          '<a href="https://www.youtube.com/watch?v=qd1RXxf2LsQ">Watch</a>';
      ...
      if (!titleIsUseful) {
        ...
        if (content != null && content.title.isNotEmpty) {
          print(
              '✅ Successfully fetched title from webpage: "${content.title}"');
